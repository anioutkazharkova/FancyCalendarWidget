...
public class ListTimeUpdateService extends Service {
	...
	private void updateTime() {
		...
		ComponentName smallComponent = new ComponentName(
				getApplicationContext(), SmallCalendarWidget.class);
		ComponentName listComponent = new ComponentName(
				getApplicationContext(), ListWidgetProvider.class);

		ComponentName extendedComponent = new ComponentName(
				getApplicationContext(), ExtendedCalendarWidget.class);
			...
			for (int widgetId : listIds) {
				try {
					ListWidgetProvider.updateWidgetListView(
							getApplicationContext(), manager, widgetId);
				} catch (Exception e) {
